...
class DryRunSnapshotEvaluator(SnapshotEvaluator):
    ...
    def get_dry_run_summary(self) -> t.Dict[str, t.Any]:
        ...
        successful = [s for s in self.simulated_executions if s.get("action") == "would_execute"]
        failed = [s for s in self.simulated_executions if s.get("action") == "would_fail"]
...
class EnhancedContext:
    ...
    def dry_run_evaluator(self) -> DryRunSnapshotEvaluator:
        ...
        if not self._dry_run_evaluator:
            ...
            ddl_concurrent_tasks = getattr(self._context.config, 'ddl_concurrent_tasks', 1)
        ...
        *,
        ...
        try:
            ...
            scheduler = self._context.scheduler(
                ...
                snapshot_evaluator=self.dry_run_evaluator
            )
